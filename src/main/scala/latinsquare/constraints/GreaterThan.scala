...
class GreaterThan(source : Cell, target : Cell, maxValue : Int, position : String)
  extends Relation(source, target, maxValue, position) {
    override def checkUpdate(value: Int): Boolean = ???

    override def update(oldValue: Int, newValue: Int): Unit = ???

    override def markup: MarkUp = ???
}
